...
d3
  ...
  .range(["#5E354A","#E16B1C","#DC9FD4","#0D8661","#33A6B8","#81C7D4"])
  .unknown("#BEBEBE")
)}
...
function _sunburst1(partition,data,d3,radius,innerCircleQuestion,outerCircleQuestion,width,color,arc,mousearc)
{
  const root = partition(data);
  const svg = d3.create("svg");
  // Make this into a view, so that the currently hovered sequence is available to the breadcrumb
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("viewof sunburst1")).define("viewof sunburst1", ["partition","data","d3","radius","innerCircleQuestion","outerCircleQuestion","width","color","arc","mousearc"], _sunburst1);
  main.variable(observer("sunburst1")).define("sunburst1", ["Generators", "viewof sunburst1"], (G, _) => G.input(_));
  return main;
}
